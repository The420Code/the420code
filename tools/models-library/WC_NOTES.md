...
5. **No "Listen — soon"** (G, the same day: "remove listen - soon for now"). The strap reads Read · Download. When a chapter has audio, `strap()` says where to put Listen back. `llms.txt` has a section on the library, with every book and PDF by its full address.
6. **No images, and every row closed on arrival** (G, the same day: "i will not add images"; "all chapters and sections are collapsed when one open the page"). The placeholder drawing and `hero()` are gone. `details()` opens nothing by default. A chapter shows its title, its line and its opening, then closed rows, then its closing paragraphs. A `#hash` link still opens the row it points into.
7. **The way back, on every page** (G, the same day). Between Previous and Next, `up()` writes "Back to" with the book's contents and Ø Models. The first chapter has no Previous; the epilogue has no Next; a book page has only the way back to Ø Models (`nav.pn.solo`). On a phone, Previous and Next sit side by side and the way back sits under them.
