# AP41 — source of record

**Verified 7 September 2026** by the paragraph-level test of record, adopted on
that date. Every source paragraph of 60 characters or more is present
in the published PDF, and every published sentence of 60 characters or more is
present in the source. 13 front-matter item(s) — the paper's own
Contents lines and heading runs — are not divergences; the checker matches them
against the published paper's own Contents block and sets them aside.

| | |
|---|---|
| Published PDF | `/AP41_The_Loop.pdf` |
| sha256 | `e749bcbad1f578d81817f22ffe7065f3eaa272cb2f7ac1db81bdb878c11441ca` |
| Source of record | `AP41_The_Loop_FINAL.docx` |
| Found in | Formatted |
| Paragraphs compared | 150 |

Harvest from this file. If the published PDF is replaced, this note and the
digest are re-verified at that republish.

*Frozen files are never edited; corrections supersede, dated, in the open.*
